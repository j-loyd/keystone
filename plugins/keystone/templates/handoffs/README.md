...
The **repo-scoped continuity index** seeded into a target repo at `docs/handoffs/README.md`: the
one file `/pickup` reads _first_ to decide what to resume. It is a plain Markdown file any harness
(Claude Code, Codex CLI, …) can read and write — file-based continuity is the cross-harness
baseline; hooks are Claude-Code-only enforcement teeth that degrade gracefully. This template
mirrors the live index the way `plugins/keystone/templates/plans/README.md` mirrors
...
| ------------ | ----------------------------- | --------------------------- | ------------------------ |
| `YYYY-MM-DD` | `docs/handoffs/YYYY-MM-DD.md` | open / resumed / superseded | `<one-line what's next>` |
...
| ------------------ | ------------------------------- | ----------------- |
| `docs/plans/<...>` | `docs/plans/<...>/RUN-STATE.md` | building / paused |
